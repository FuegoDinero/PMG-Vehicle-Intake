...
## V2 fixes in this build

- OCR now runs multiple image-preprocessing passes and validates UK-style registrations before accepting a result.
- The camera remains a live viewfinder; no camera image is stored by the app.
- 1× / 2× / 3× camera zoom and focus controls remain available.
- Vehicle lookup uses the PMG Vehicle Intake Pages Function at `/api/vehicle`.
- The Pages Function explicitly handles POST and OPTIONS requests and reports upstream DVSA errors instead of hiding them.
- DVSA uses the documented OAuth client-credentials flow and GET registration endpoint.
- Existing Cloudflare secret names are supported with aliases for client ID/client secret.
- Work destinations remain separate from on-site locations.
